...
Winner.attachSchema(Schemas.AnswersScheme);
...
Meteor.methods({
    ballotInsert: function (ballotAttributes) {
        if (!ballotAttributes.name) {
            throw new Meteor.Error('invalid-ballot', "You must set your name");
        }


        var ballotWithSameLink = Ballot.findOne({name: ballotAttributes.name});
        if (ballotWithSameLink) {
            throw new Meteor.Error('invalid-ballot', "Name already exists");
