...
Schemas.AnswersScheme = new SimpleSchema({
    picture: {
        type: String,
        label: "Best Motion Picture",
        optional: true,
...
Meteor.methods({
    winnerUpdate: function () {
        ...
        var ballots = Ballot.find().fetch();

        Ballot.find().forEach(function(ballot) {
            var points = 0;
            for (var key in winner) {
                if (key != "_id" && key != "counted_keys") {
                    var first = ballot.first[key];
                    var second = ballot.second[key];

                    if (typeof first !== 'undefined' && first == winner[key]) {
                        points += 10;
                    }

                    if (typeof first !== 'undefined'&& second == winner[key]) {
                        points += 5;
                    }
                }
            }

            if (ballot.points != points) {
                Ballot.update(ballot._id, {$set: {points: points}});
            }
        });
        //for (var key in winner) {
        //    if (winner.counted_keys.indexOf(key) == -1) {
        //        var string = 'first.' + key;
        //        var cursor = Ballot.find({string: winner[key]});
        //        cursor.forEach(function(ballot) {
        ...
        //        });
        //
        //        Ballot.find('second.' + key + '=' + winner[key]).forEach(function(ballot) {
        //            var ballotID = ballot._id;
        //            var updatedProperties = {
        ...
        //        });
        //
        //        Winner.update(winner._id, {
        //            $push: {counted_keys: key}
        //        });
        //    }
        //}
    }
